...
export default function ChatHeader({
    ...
    return (
        <header className="shrink-0 w-full p-4 md:px-6 flex items-center justify-between z-30 bg-bg-base transition-colors">
                                ...
                                >
                                    <div className="max-h-[300px] overflow-y-auto custom-scrollbar">
                                        {availableModels.length === 0 ? (
                                            <div className="p-4 text-center text-sm text-text-secondary">無可用模型，請掃描目錄。</div>
                                        ) : (
                                            availableModels.map(model => (
                                                <button
                                                    ...
                                                    className={`w-full text-left px-4 py-3 mb-1 rounded-full flex items-center justify-between transition-colors ${selectedModel?.name === model.name ? 'bg-bg-base ' : 'hover:bg-bg-base '}`}
                                                >
                                                    <div className="flex flex-col min-w-0 pr-3">
                                                        <span className={`font-semibold text-[14px] break-all ${selectedModel?.name === model.name ? 'text-text-primary ' : 'text-text-primary '}`}>{model.name}</span>
                                                        <span className="text-[12px] text-text-secondary mt-1 flex items-center gap-2">
                                                            <span>{model.size_gb.toFixed(1)} GB</span>
                                                            {model.quantization && (
                                                                <span className="bg-bg-surface/50 px-1.5 py-0.5 rounded-full text-[10px] uppercase">{model.quantization}</span>
                                                            )}
                                                        </span>
                                                    </div>
                                                    {selectedModel?.name === model.name && (
                                                        <svg className="w-5 h-5 ml-2 text-corphia-bronze shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M5 13l4 4L19 7"></path></svg>
                                                    )}
                                                </button>
                                            ))
                                        )}
                                    ...
                                    <div className="mt-2 pt-2 border-t border-border-subtle">
                                        <button 
                                            ...
                                            className="w-full text-left px-4 py-3 rounded-full flex items-center gap-3 transition-colors hover:bg-bg-base text-text-secondary group"
                                        >
                            ...
                            <>
                                ...
                                >
                                    ...
                                    <button 
                                        ...
                                        className="w-full text-left px-3 py-2.5 rounded-[8px] flex items-center gap-3 transition-colors hover:bg-emerald-50 text-emerald-600 active:bg-emerald-100"
                                    >
                                    ...
                                    <button 
                                        ...
                                        className="w-full text-left px-3 py-2.5 rounded-[8px] flex items-center gap-3 transition-colors hover:bg-red-50 text-red-500 active:bg-red-100"
                                    >
