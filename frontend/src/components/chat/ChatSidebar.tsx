    ...
    return (
        <aside
            className={`${sidebarOpen ? 'w-[75vw] max-w-[260px] md:w-[280px] translate-x-0' : 'w-0 -translate-x-full md:w-[72px] md:translate-x-0'
                } overflow-hidden bg-[#F5F3EF] dark:bg-dark-bg-primary rounded-r-[20px] md:rounded-[38px] md:border-r border-light-border-secondary dark:border-white/5 transition-[width,transform] duration-300 ease-in-out shrink-0 flex flex-col z-50 absolute md:relative h-full md:h-[calc(100vh-24px)] md:my-3 md:ml-3 shadow-sm dark:shadow-2xl dark:shadow-black`}
        >
            ...
            <div className={`flex items-center w-full p-4 pb-1 h-[60px] shrink-0 transition-opacity duration-300 ${sidebarOpen ? 'justify-between' : 'justify-center md:px-0'}`}>
                ...
                <button
                    ...
                    className={`hidden md:flex rounded-full text-light-text-secondary hover:text-light-text-primary dark:text-light-text-muted dark:hover:text-gray-200 hover:bg-light-bg-tertiary dark:hover:bg-light-bg-primary/5 transition-all duration-200 shrink-0 items-center justify-center ${
            ...
            <div className={`w-full transition-all duration-300 lg:p-4 p-3 flex flex-col gap-4 py-4 ${!sidebarOpen ? 'items-center' : ''}`}>
                ...
                {sidebarOpen ? (
                    ...
                    >
                        <div
                            className="bg-light-bg-primary dark:bg-[#2A2420] shadow-sm rounded-full border border-gray-100 dark:border-white/5"
                            style={{
                        ...
                        <button type="button" onClick={() => setChatMode('general')}
                            ...
                            className={`flex-1 py-1.5 text-[14px] text-center rounded-full font-semibold transition-colors duration-300 ${chatMode === 'general' ? 'text-light-text-primary dark:text-dark-text-primary' : 'text-light-text-secondary dark:text-light-text-secondary hover:text-light-text-primary dark:hover:text-gray-300'}`}
                        >{t('chat.general')}</button>
                        <button type="button" onClick={() => setChatMode('project')}
                            style={{ position: 'relative', zIndex: 2, WebkitTapHighlightColor: 'transparent' }}
                            className={`flex-1 py-1.5 text-[14px] text-center rounded-full font-semibold transition-colors duration-300 ${chatMode === 'project' ? 'text-light-text-primary dark:text-dark-text-primary' : 'text-light-text-secondary dark:text-light-text-secondary hover:text-light-text-primary dark:hover:text-gray-300'}`}
                    ...
                    >
                        <div className="absolute bg-light-bg-primary dark:bg-[#2A2420] rounded-full shadow-sm border border-gray-100 dark:border-white/5"
                            style={{
                        ...
                        />
                        <div className={`absolute flex items-center justify-center z-10 transition-colors duration-300 ${chatMode === 'general' ? 'text-light-text-primary dark:text-dark-text-primary' : 'text-light-text-secondary dark:text-light-text-secondary'}`}
                        ...
                        <div className={`absolute flex items-center justify-center z-10 transition-colors duration-300 ${chatMode === 'project' ? 'text-light-text-primary dark:text-dark-text-primary' : 'text-light-text-secondary dark:text-light-text-secondary'}`}
                ...
                {chatMode === 'general' ? (
                    <>
                        <div className="mb-2 pl-3 mt-1">
                            <span className="text-[12px] text-light-text-secondary tracking-wider font-medium">{t('chat.generalChat')}</span>
                        </div>
                        <div className="space-y-1 transition-colors px-2">
                            {(() => {
                                const filtered = conversations.filter(c => !c.settings?.isProject)
                                if (filtered.length === 0) return <p className="text-light-text-muted text-[13px] py-4 pl-3">{t('chat.noChats')}</p>
                                return filtered.map((conv) => (
                                    <div key={conv.id} onClick={() => selectConversation(conv)}
                                        className={`w-full flex items-center justify-between text-left px-3 py-2 rounded-xl text-[14px] transition-colors group cursor-pointer ${currentConversationId === conv.id
                                            ? 'bg-[#EFEAE4] dark:bg-dark-bg-secondary text-[#8B7355] dark:text-dark-text-primary font-medium'
                                            : 'text-[#6B6B6B] dark:text-light-text-muted hover:bg-[#EFEAE4] dark:hover:bg-ios-dark-gray5 hover:text-[#8B7355] dark:hover:text-gray-200'}`}
                                    >
                                        ...
                                        <div className={`flex items-center gap-1 transition-opacity ${activeMenuConvId === conv.id ? 'opacity-100' : 'opacity-100 md:opacity-0 md:group-hover:opacity-100'}`}>
                                            <button onClick={(e) => handleOpenMenu(e, conv.id)}
                                                className={`p-1.5 rounded-full hover:bg-light-bg-tertiary dark:hover:bg-ios-dark-gray3 ${activeMenuConvId === conv.id ? 'bg-light-bg-tertiary dark:bg-ios-dark-gray3 text-light-text-primary dark:text-dark-text-primary' : 'text-light-text-muted'}`}
                                                title="選項">
                    ...
                    <>
                        <div className="mb-2 pl-3 mt-1">
                            <span className="text-[12px] text-light-text-muted dark:text-light-text-secondary tracking-wider font-medium">{t('chat.projectFolderLabel')}</span>
                        </div>
                        {(() => {
                            ...
                            if (allFolders.length === 0) {
                                return (
                                    <div className="px-2 space-y-1 transition-colors">
                                        <p className="text-light-text-muted text-[13px] py-4 pl-3">{t('chat.noProjects')}</p>
                                    </div>
                                )
                            }
                                                ...
                                                >
                                                    <div className="flex items-center gap-1 min-w-0">
                                                        <button onClick={(e) => { e.stopPropagation(); setExpandedFolders(prev => { const next = new Set(prev); if (next.has(folderName)) next.delete(folderName); else next.add(folderName); return next }) }}
                                                            className={`p-0.5 rounded-md hover:bg-light-bg-tertiary dark:hover:bg-light-bg-primary/10 transition-colors ${isExpanded ? 'text-light-text-secondary dark:text-light-text-muted' : 'text-light-text-muted dark:text-light-text-secondary'}`}>
                                                    ...
                                                    <div className="flex items-center gap-1 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
                                                        <button onClick={(e) => createConvInFolder(folderName, e)} className="p-1 text-light-text-muted hover:text-light-text-primary dark:hover:text-gray-200 rounded hover:bg-light-bg-tertiary dark:hover:bg-ios-dark-gray4" title="在此資料夾新增對話">
                                                            <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}><path strokeLinecap="round" strokeLinejoin="round" d="M12 4v16m8-8H4" /></svg>
                                                        </button>
                                                        <button onClick={(e) => handleDeleteFolder(folderName, e)} className="p-1 text-light-text-muted hover:text-red-500 rounded hover:bg-red-50 dark:hover:bg-red-900/20" title="刪除資料夾及內容">
                                                            ...
                                                            <div key={conv.id} className="relative">
                                                                <div onClick={() => selectConversation(conv)}
                                                                    className={`w-full flex items-center justify-between text-left px-3 py-1.5 rounded-xl text-[13px] transition-colors group cursor-pointer border border-transparent ${currentConversationId === conv.id
                                                                        ? 'bg-[#EFEAE4] dark:bg-dark-bg-secondary text-[#8B7355] dark:text-dark-text-primary font-medium border-transparent'
                                                                        : 'text-[#6B6B6B] dark:text-light-text-muted hover:text-[#8B7355] dark:hover:text-gray-200 hover:bg-[#EFEAE4] dark:hover:bg-ios-dark-gray3'}`}
                                                                >
                                                                    <span className="truncate pr-2">{conv.title}</span>
                                                                    <div className={`flex items-center gap-1 transition-opacity ${activeMenuConvId === conv.id ? 'opacity-100' : 'opacity-100 md:opacity-0 md:group-hover:opacity-100'}`}>
                                                                        <button onClick={(e) => handleOpenMenu(e, conv.id)}
                                                                            className={`p-1 rounded-full hover:bg-light-bg-tertiary dark:hover:bg-ios-dark-gray2 ${activeMenuConvId === conv.id ? 'bg-light-bg-tertiary dark:bg-ios-dark-gray2 text-light-text-primary dark:text-dark-text-primary' : 'text-light-text-muted'}`}
                                                                            title="選項">
            ...
            <div className={`w-full transition-all duration-300 p-3 mt-auto flex flex-col ${!sidebarOpen ? 'items-center' : ''}`}>
                <button 
                    ...
                    title="前往設定"
                    className={`relative flex items-center bg-transparent hover:bg-light-bg-secondary dark:hover:bg-ios-dark-gray4 transition-colors text-left overflow-hidden group ${sidebarOpen ? 'w-full px-3 py-2 justify-start rounded-full gap-3' : 'w-12 h-12 justify-center rounded-full shrink-0 gap-0'}`}
                >
                    ...
                    {sidebarOpen && (
                        <div className="flex-1 min-w-0">
                            <p className="font-semibold text-[14px] text-light-text-primary dark:text-dark-text-primary truncate text-left m-0 leading-snug">{user?.name || 'Local User'}</p>
                        </div>
                    )}
                </button>
            </div>
