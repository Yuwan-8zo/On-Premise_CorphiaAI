...
const tabs: Array<{ id: AdminSection; label: string; icon: React.ComponentType<{ className?: string }> }> = [
    { id: 'overview', label: 'Á∏ΩË¶Ω', icon: Gauge },
    { id: 'users', label: '‰ΩøÁî®??, icon: Users },
    { id: 'models', label: 'Ê®°Â?', icon: Cpu },
    { id: 'audit', label: 'Á®ΩÊ†∏', icon: FileText },
    { id: 'system', label: 'Á≥ªÁµ±', icon: Activity },
    { id: 'tenants', label: 'ÁßüÊà∂', icon: Building2 },
]
...
export default function Admin() {
    ...
    const handleDeleteUser = async (item: UserData) => {
        if (!window.confirm(`Á¢∫Â?Ë¶ÅÂà™??${item.name}ÔºüÊ≠§?ç‰??ÉÁßª?§Ë©≤‰ΩøÁî®?Ö„ÄÇ`)) return
        try {
    ...
    const handleToggleTenantStatus = async (item: Tenant) => {
        const action = item.is_active ? '?úÁî®' : '?üÁî®'
        if (!window.confirm(`Á¢∫Â?Ë¶?{action}ÁßüÊà∂ ${item.name}Ôºü`)) return
        try {
        ...
        <div className="min-h-screen overflow-hidden bg-bg-base text-text-primary">
                ...
                <header className="mb-5 flex flex-col gap-4 rounded-[30px] border border-border-subtle/70 bg-bg-base/78 p-4 shadow-xl backdrop-blur-2xl dark:shadow-black/40 md:flex-row md:items-center md:justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <h1 className="text-2xl font-semibold tracking-tight text-text-primary md:text-3xl">ÁÆ°Á?ÂæåÂè∞</h1>
                        </div>
                    </div>
                ...
                <main className="grid flex-1 gap-5 lg:grid-cols-[260px_minmax(0,1fr)]">
                    ...
                    <div className="min-w-0 space-y-5 pb-8">
                        ...
                        {activeSection === 'users' && (
                            <Panel className="overflow-hidden">
                                <SectionHeader
                                    title={`‰ΩøÁî®?ÖÁÆ°??(${users.length})`}
                                    eyebrow={`${activeUsers} active operators`}
                                    action={<ActionButton onClick={handleAddUser}><Plus className="h-4 w-4" />?∞Â?‰ΩøÁî®??/ActionButton>}
                                />
                                                ...
                                                <tr key={item.id} className="transition hover:bg-bg-base/45 /[0.035]">
                                                    ...
                                                    <td className="px-6 py-4">
                                                        <div className="flex justify-end gap-2">
                                                            <ActionButton variant="secondary" onClick={() => handleEditUser(item)}><UserRoundCog className="h-4 w-4" />Á∑®ËºØ</ActionButton>
                                                            <ActionButton variant="danger" onClick={() => handleDeleteUser(item)}><Trash2 className="h-4 w-4" />?™Èô§</ActionButton>
                                                        </div>
                                                    </td>
                                                </tr>
                                            ))}
                            ...
                            <div className="space-y-5">
                                <Panel>
                                    <SectionHeader
                                        ...
                                        action={<ActionButton onClick={handleRefreshModels} disabled={isLoadingModels}><RefreshCw className="h-4 w-4" />?çÊñ∞?ÉÊ?</ActionButton>}
                                    />
                                    <div className="grid gap-4 p-5">
                                        {isLoadingModels ? (
                                            ...
                                            <div key={model.name} className="flex flex-col gap-4 rounded-[24px] border border-border-subtle/70 bg-bg-base/55 p-5 /10 /[0.045] md:flex-row md:items-center md:justify-between">
                                                ...
                                                {!model.is_current && <ActionButton variant="secondary" onClick={() => handleSelectModel(model.name)}>?∏Áî®Ê®°Â?</ActionButton>}
                                            </div>
                                        ))}
                                    </div>
                                </Panel>
                                    ...
                                    <div className="flex flex-col gap-3 xl:flex-row xl:items-end">
                                        <Field label="Search">
                                            <div className="flex gap-2">
                                                ...
                                                <ActionButton onClick={handleAuditSearch}><Search className="h-4 w-4" />?úÂ?</ActionButton>
                                            </div>
                                        </Field>
                                        <Field label="Action">
                                            <select className={inputClass} value={auditFilter.action || ''} onChange={(event) => handleAuditFilterChange('action', event.target.value)}>
                                                ...
                                                <option value="login_success">?ªÂÖ•?êÂ?</option>
                                                <option value="login_failed">?ªÂÖ•Â§±Ê?</option>
                                                <option value="user_update">‰ΩøÁî®?ÖÊõ¥??/option>
                                                <option value="document_upload">?á‰ª∂‰∏äÂÇ≥</option>
                                                <option value="document_delete">?á‰ª∂?™Èô§</option>
                                            </select>
                                        </Field>
                        ...
                        {activeSection === 'system' && (
                            <div className="grid gap-5 xl:grid-cols-2">
                                <Panel>
                                    <SectionHeader title="Á≥ªÁµ±Ë≥áË?" eyebrow="Runtime" />
                                ...
                                <Panel>
                                    <SectionHeader title="Á∂≠È??ç‰?" eyebrow="Maintenance" />
                                    <div className="space-y-3 p-5">
                                        <ActionButton variant="secondary"><RefreshCw className="h-4 w-4" />Ê∏ÖÈô§Âø´Â?</ActionButton>
                                        <ActionButton variant="secondary"><SlidersHorizontal className="h-4 w-4" />?çÊñ∞Á¥¢Â??ëÈ?</ActionButton>
                                        <ActionButton variant="danger"><CircleAlert className="h-4 w-4" />?çÂ??çÂ?</ActionButton>
                                    </div>
                                </Panel>
                            </div>
                        )}
                        ...
                        {activeSection === 'tenants' && (
                            <Panel className="overflow-hidden">
                                <SectionHeader
                                    title={`ÁßüÊà∂ÁÆ°Á? (${tenants.length})`}
                                    eyebrow={`${activeTenants} active tenants`}
                                    action={<ActionButton onClick={handleAddTenant}><Plus className="h-4 w-4" />?∞Â?ÁßüÊà∂</ActionButton>}
                                />
                                    ...
                                    {isLoadingTenants ? (
                                        ...
                                        <div key={item.id} className="rounded-[24px] border border-border-subtle/70 bg-bg-base/55 p-5 /10 /[0.045]">
                                            ...
                                            <div className="mt-6 flex justify-end gap-2">
                                                <ActionButton variant="secondary" onClick={() => handleToggleTenantStatus(item)}>{item.is_active ? '?úÁî®' : '?üÁî®'}</ActionButton>
                                                <ActionButton onClick={() => handleEditTenant(item)}>Á∑®ËºØ</ActionButton>
                                            </div>
                                        </div>
                                    ))}
                                </div>
            ...
            {isUserModalOpen && (
                <ModalFrame onClose={() => setIsUserModalOpen(false)}>
                    <h3 className="mb-6 text-xl font-semibold">{currentEditingUser ? 'Á∑®ËºØ‰ΩøÁî®?? : '?∞Â?‰ΩøÁî®??}</h3>
                    <form onSubmit={handleUserSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-3 pt-4">
                            <ActionButton variant="secondary" onClick={() => setIsUserModalOpen(false)}>?ñÊ?</ActionButton>
                            <ActionButton disabled={isSubmittingUser}>{isSubmittingUser ? '?≤Â?‰∏?..' : '?≤Â?'}</ActionButton>
                        </div>
                    </form>
                </ModalFrame>
            )}
            ...
            {isTenantModalOpen && (
                <ModalFrame onClose={() => setIsTenantModalOpen(false)}>
                    <h3 className="mb-6 text-xl font-semibold">{currentEditingTenant ? 'Á∑®ËºØÁßüÊà∂' : '?∞Â?ÁßüÊà∂'}</h3>
                    <form onSubmit={handleTenantSubmit} className="space-y-4">
                        ...
                        <div className="flex gap-3 pt-4">
                            <ActionButton variant="secondary" onClick={() => setIsTenantModalOpen(false)}>?ñÊ?</ActionButton>
                            <ActionButton disabled={isSubmittingTenant}>{isSubmittingTenant ? '?≤Â?‰∏?..' : '?≤Â?'}</ActionButton>
                        </div>
                    </form>
                </ModalFrame>
            )}
