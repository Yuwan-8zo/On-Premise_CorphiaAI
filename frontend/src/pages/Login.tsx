 ...
 * ?ªÂÖ•/Ë®ªÂ??ÅÈù¢
 * 
 * Á≤æÁ¢∫Âæ©Âàª?®Êà∂Ë®≠Ë?Ôº?:1 Ê≠?ñπÂΩ¢Âç°??
 */
...
export default function Login() {
    ...
    const [showQR, setShowQR] = useState(false) // ?∞Â? QR Modal ?Ä??
    const langMenuRef = useRef<HTMLDivElement>(null)
    
    // ?±Áî®?ÑËº∏?•Á???
    const [email, setEmail] = useState('')
    const [password, setPassword] = useState('')
    // Ë®ªÂ?Â∞àÁî®?ÑÁ¢∫Ë™çÂ?Á¢ºÁ???
    ...
    useEffect(() => {
        ...
        // ?üÂ??üÈ??†Âø´Ëº™Ë©¢?üÂ∫¶ (ÊØ?3 Áß?ÔºåÊ??üÈÄ??ÂæåÊÅ¢Âæ?30 ÁßíËº™Ë©?
    ...
    // Â¶ÇÊ? 10 ÁßíÂ??ΩÈÄ??‰∏äÔ?È°ØÁ§∫?åË∑≥?éÁ?ÂæÖ„ÄçÊ???
    useEffect(() => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (status === 429) {
                // ?üÁ??êÂà∂ÔºöÂ?Á´ØÂ??ÇÂ???detail ??error.message
                setError(data?.detail || data?.error?.message || 'Ë´ãÊ??éÊñº?ªÁ?ÔºåË?Á®çÂ??çË©¶')
            } else if (status === 422) {
                // Pydantic È©óË??ØË™§ (ÂØÜÁ¢ºÂº∑Â∫¶‰∏çÁ¨¶?àÁ?)
                // ÂæåÁ´Ø?ûÂÇ≥?ºÂ?: { error: { details: [{ field, message }] } }
                ...
                if (details && details.length > 0) {
                    ...
                    if (pwdError) {
                        // msg ?ºÂ???Value error, ÂØÜÁ¢º?∑Â∫¶?≥Â?...; ÂØÜÁ¢º?Ä?ÖÂê´..."
                        const cleaned = pwdError.message.replace(/^Value error,\s*/i, '')
                        setError(cleaned)
                    } else {
                        setError(details.map(d => d.message.replace(/^Value error,\s*/i, '')).join('Ôº?))
                    }
                } else {
    ...
    return (
        <div className="min-h-screen flex bg-bg-base transition-colors duration-300 relative overflow-hidden">
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center gap-2 text-center">
                                ...
                                <p className="text-text-secondary">
                                    Ê≠?ú®?öÈ?ÂæåÁ´Ø?çÂ??áÂ?ËºâÂ§ßË™ûË?Ê®°Â?ÔºåË?Á®çÂÄ?..
                                </p>
            ...
            {/* ?Ä?Ä ?®Ëû¢Âπ?QR Code ?õÂ??åÊôØ Modal ?Ä?Ä */}
            <AnimatePresence>
                {showQR && (
                    <motion.div
                        initial={{ opacity: 0 }}
            ...
            {/* ?Ä?Ä Â∑¶ÂÅ¥ÔºöÂ??å‰?Á¥?(Ê°åÈù¢ 50%) ?Ä?Ä */}
            <div className="hidden lg:flex lg:w-1/2 flex-col p-8 relative z-10">
                ...
                <button
                    ...
                    title={t('auth.scanToDownload', '?ÉÁ¢º‰∏ãË?Ë°åÂ???)}
                >
            ...
            {/* ?Ä?Ä ?≥ÂÅ¥ÔºöÁôª?•Ë°®??(Ê°åÈù¢ 50%ÔºåÊ?Ê©?100%) ?Ä?Ä */}
            <div className="w-full lg:w-1/2 flex flex-col z-10">
                {/* ?ÇÈÉ®?ÄÂ°?(?ãÊ??àÈ°ØÁ§∫Â∑¶?¥Á??ãË??≥ÂÅ¥?âÈ?ÔºåÊ??¢Á??™È°ØÁ§∫Âè≥?¥Ê??? */}
                ...
                {/* ?ãÊ??àÂ??åÊ?È°?(?ÖÂú®Ë°åÂ?Ë£ùÁΩÆÈ°ØÁ§∫) */}
                ...
                <div className="flex-1 flex items-start lg:items-center justify-center px-6 lg:px-8 pb-12">
                    {/* ?°Á??¨È?Ôº?:1 Ê≠?ñπÂΩ¢Ô?flex spacer Á≤æÊ??ÇÁõ¥?ÜÈ? */}
                    <form
                    ...
                    >
                    <LayoutGroup>
                        {/* ?Ä?Ä Pill Tab ?áÊ?ÔºàÊ??ïË??ØÔ? ?Ä?Ä */}
                        <div
                        ...
                        {/* ?Ä?Ä Spacer B2ÔºàÂ?ÁµÇÂú® DOMÔºåÂ?Ê≠•Â???flexGrowÔº??Ä?Ä */}
                        <motion.div
                        ...
                        {/* ?Ä?Ä Confirm PasswordÔºàÂ?ÁµÇÂú® DOMÔºåÂ?Ê≠•Â??´Ô? ?Ä?Ä */}
                        <motion.div
                        ...
                        {/* ?Ä?Ä Spacer C (Â∫ïÈÉ®Ë¶ñË¶∫Ë£úÂ? 1.15) ?Ä?Ä */}
                        <motion.div layout className="flex-[1.15]" />

                        {/* Â∫ïÂ±§?âÈ??áÈåØË™§Ê?Á§∫Â?Â°?*/}
