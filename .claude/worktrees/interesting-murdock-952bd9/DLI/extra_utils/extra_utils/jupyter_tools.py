...
import re
try: 
    from .nb_loader import NotebookLoader, concatenate_cells_to_md
except: 
    from nb_loader import NotebookLoader, concatenate_cells_to_md
...
class JupyterCell(BaseModel):
    ...
    def transform(cls, raw: Union[str, List[str]]) -> str:
        """Cleans and normalizes source content to handle common LLM output issues."""
        if isinstance(raw, list):
            raw = "\n".join([line.lstrip() for line in raw])
        raw = re.sub(r"\\{2,}", r"\\", raw).replace("\\n", "\n")
        return raw
...
class JupyterChunk(BaseModel):
    ...
    filename: Optional[str] = Field("temp.ipynb")

    work_dir: ClassVar[str] = "./temp_nbs"


    @field_validator("cells", mode="before")
    def transform(cls, raw: List[JupyterCell]) -> List[JupyterCell]:
        """Cleans and normalizes source content to handle common LLM output issues."""
        if not raw:
            return []
        cleaned_cells = []
        ...
        for cell in raw:
            if isinstance(cell, dict):
                cell = JupyterCell(**cell)
            cell.source = cell.source.strip()  # Remove extra whitespace
            if cell.cell_type == "code":
                if cell.source.strip().startswith("```python"):
                    cell.source == cell.source.replace("```python", "").lstrip()
                if cell.source.strip().endswith("```"):
                    cell.source == cell.source.replace("```", "").rstrip()
            # Merge adjacent code cells
            if prev_cell and cell.cell_type == "code" and prev_cell.cell_type == "code":
                prev_cell.source += "\n\n" + cell.source
            else:
    ...
    def to_markdown(self, delim="\n", as_list=False):
        md_list = [concatenate_cells_to_md(c) for c in self.to_json()]
        if as_list:
            return md_list
