...
# Output goes to dist/corphia-server/, then copied into Tauri sidecar location.
#
# Usage:
#   cd backend
...
$TauriBinariesDir = Join-Path $BackendRoot "..\frontend\src-tauri\binaries"
...
Write-Host "  1. Re-add externalBin to tauri.conf.json (or run build_installer.ps1)"
Write-Host "  2. cd frontend; npx tauri dev   (no separate uvicorn needed)"
Write-Host ""
